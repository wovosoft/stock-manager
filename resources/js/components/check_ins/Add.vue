<template>
    <b-modal @hidden="$router.go(-1),$emit('reset',true)"
             ...
             no-body
             :title="__((form.id?'edit':'add')+'_check_in',(form.id?'Edit ':'Add ')+'Check In')"
             lazy>
        <form @submit.prevent="hitSubmit" ref="theForm">
            ...
            <b-form-group :label="__('note','Note')">
                <b-form-textarea :placeholder="__('check_in_note','Check-in Note')" v-model="form.note"/>
            </b-form-group>
            <vue-select
                :placeholder="__('search_and_select_product','Search & Select Products')"
            ...
            <b-table
                ...
                <template v-slot:cell(quantity)="row">
                    <b-form-input type="number" size="sm"
                                  ...
                                  v-model="row.item.quantity" :min="0"/>
                </template>
        ...
        <template v-slot:modal-footer="{cancel}">
            <b-button variant="primary" ref="submitBtn" @click="onSubmit">P{{__('submit','SUBMIT')}}</b-button>
