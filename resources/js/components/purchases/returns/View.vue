<template>
    <b-modal @hidden="$router.go(-1)" visible
             ...
             :title="__('view_return','View Return')">
        <b-row>
            <b-col>
                <b-table small bordered hover striped
                         head-variant="dark"
                         :items="obj2Table(the_item,['deleted_at','supplier_id','product_id'])"
                         :fields="[
                            {
                    ...
                    <template v-slot:cell(value)="row">
                        ...
                        <template v-else-if="['supplier','product'].includes(row.item.key)">
                            {{Object.values(row.item.value).join(' # ')}}
                        </template>
