<template>
    <div>
        <b-alert variant="danger" v-if="form.payment_amount===0" show>
            {{__('payment_is_completed_for_this_purchase',
            'Payment is completed for this Purchase. No further action required.')}}
        </b-alert>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.form.payment_amount = this.purchase.balance;
        },
        methods: {
            handleSubmit() {
                if (Number(this.form.purchase_payment) === 0) {
