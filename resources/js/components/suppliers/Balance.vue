<template>
    <div>
        <b-card :header="__('supplier_balance_sheet','Supplier Balance Sheet')" body-class="p-2">
            <vue-select
                :required="true"
            ...
            <template v-slot:footer v-if="form.supplier">
                <b-button size="sm"
                          variant="primary"
                ...
                <b-button size="sm"
                          ...
                          :title="__('make_due_purchase_payment_from_available_fund','Make Due Purchase Payment From Available Fund')"
                          variant="dark">
        ...
        <b-modal id="add_fund_modal"
            ...
            <template v-slot:default="{hide}">
                <add-fund
                    @success="v=>{if(v) hide();}"
                    @refresh="v=>getSupplier(form.supplier.id)"
                    v-if="form.supplier && form.supplier.id"
        ...
        <b-modal id="due_sales_modal"
                 ...
                 size="xl"
                 hide-footer
                 :title="__('due_purchases','Due Purchases')"
                 header-bg-variant="dark"
                 header-text-variant="light">
            <template v-slot:default="{hide}">
                <due-purchases
                    @message="v=>msgBox(v)"
                    @success="v=>{if(v) getSupplier(output.id)}"
                    :supplier-record="output"/>
            </template>
        </b-modal>
    </div>
</template>
<script>
    ...
    import DuePurchases from "@/components/purchases/DuePurchases";
    ...
    export default {
        ...
        methods: {
            ...
            startCase,
            supplierSelected(v) {
                this.form.supplier_id = v ? v.id : null;
                if (v && v.id) {
                    this.getSupplier(v.id);
                }
            },
            getSupplier(id) {
