<template>
    <div>
        ...
        <b-modal id="addModal" :title="add_modal_title " size="xl" header-bg-variant="dark"
            ...
            <template v-slot:modal-footer="{close}">
                <b-button-group class="float-right">
                    <b-button variant="primary" @click="$refs.ItemAdd.hitSubmit()">
                        {{__('submit','SUBMIT')}}
                    </b-button>
        ...
        <b-modal id="viewModal" :title="view_modal_title" size="lg" header-bg-variant="dark" header-text-variant="light"
            ...
            <roles :title="__('assigned_roles','Assigned Roles')" :permission_id="currentItem.id"></roles>
            <br>
            <users :title="__('assigned_users','Assigned Users')" :permission_id="currentItem.id"></users>
        </b-modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            add_modal_title: {
                type: String,
                default: _t('add_edit_permission', 'Add / Edit Permission')
            },
            view_modal_title: {
                type: String,
                default: _t('view_permission', 'View Permission')
            },
            custom_buttons: {
                type: Array,
                default() {
        ...
        data: function () {
            return {
                ...
                form: {},
                fields: [
                    {key: 'id', sortable: true, visible: true, label: _t('id', 'ID')},
                    {key: 'name', sortable: true, visible: true, label: _t('name', 'Name')},
                    {key: 'guard_name', sortable: true, visible: true, label: _t('guard', "Guard")},
                    {key: 'description', sortable: true, visible: true, label: _t('description', 'Description')},
                    {
                        key: 'created_at',
                        sortable: true,
                        visible: true,
